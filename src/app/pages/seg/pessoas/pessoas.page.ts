...
export class PessoasPage implements OnInit {
  ...
  public collection: Empresa[] = [];
  public collectionFilter: Empresa[];
  public model: Empresa = new Empresa;
  public modelPesquisa: Empresa = new Empresa;
  ...
  public collectionEmpresaSituacao: any = [{
    CodigoUnidadeSituacao: 0,
    Nome: '',
    Sigla: ''
  }];
  public collectionEmpresaTipo: any = [{
    CodigoUnidadeSituacao: 0,
    Nome: '',
    Sigla: ''
  }];
  ...
  private consultarEmpresaSituacao() {
    this.sendRequest('spCarregaEmpresaSituacao', {
      StatusCRUD: '',
      formValues: ''
    }, (resultado) => {
      this.collectionEmpresaSituacao = JSON.parse(atob(resultado.results));
    });
  }
  private consultarEmpresaTipo() {
    this.sendRequest('spCarregaEmpresaTipo', {
      StatusCRUD: '',
      formValues: ''
    }, (resultado) => {
      this.collectionEmpresaTipo = JSON.parse(atob(resultado.results));
    });
  }

  private consultarSegmentos() {
  ...
  salvar(form: NgForm) {
    ...
    const params = {
      StatusCRUD: this.model.CodigoEmpresa > 0 ? 'UPDATE' : 'CREATE',
      formValues: this.model,
    };
    this.sendRequest('spEmpresas', params, (resultado) => {
      if (params.StatusCRUD === 'CREATE') {
        ...
        this.collectionFilter = this.collection.map((item, index) => {
          if (item.CodigoEmpresa === this.model.CodigoEmpresa) {
  ...
  search() {
    ...
    if (this.modelPesquisa.CnpjCpf
      ...
      this.sendRequest('spMoradores', params, (resultado) => {
        this.collection = JSON.parse(atob(resultado.results));
        this.collectionFilter = this.collection;
        this.RecordCount = this.collectionFilter[0].RecordCount;
        this.showPages(this.PageNumber, this.RowsPage);
